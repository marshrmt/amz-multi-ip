...
- для `awg` принудительно включает IPv4-only режим, чтобы не тянуть серверный IPv6 в клиентские конфиги
...
IPv6 note:
- `awg` в этом скрипте настраивается как IPv4-only
- если после импорта клиент всё равно показывает IPv6 на сайтах проверки, это уже нативный IPv6 самого устройства или сети клиента, его нужно отключать на клиенте
...
IPv6 note:
- `awg`-часть в режиме `both` тоже принудительно переводится в IPv4-only
- если сайт всё равно видит IPv6, это не серверный AWG IPv6, а нативный IPv6 клиентского устройства/сети
