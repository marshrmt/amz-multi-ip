...
- для `awg` принудительно блокирует IPv6 leak: клиент получает `0.0.0.0/0,::/0`, а серверный IPv6 остаётся выключенным
...
IPv6 note:
- `awg` в этом скрипте принудительно блокирует IPv6 на клиенте через профиль
- под подключённым AWG IPv6 не должен работать и не должен светиться на сайтах проверки
- после изменения логики переимпортируйте свежий `vpn://`, старые профили Amnezia могут держать старые параметры
...
IPv6 note:
- `awg`-часть в режиме `both` тоже принудительно блокирует IPv6 leak через профиль
- после изменения логики переимпортируйте свежий `vpn://`, старые профили Amnezia могут держать старые параметры
